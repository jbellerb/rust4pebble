...
impl ParseCallbacks for ParseDoxygen {
    fn process_comment(&self, comment: &str) -> Option<String> {
        doxygen::Doc::new(comment).render()
    }
}
...
mod doxygen {
    ...
    fn block_type<'a>(command_token: Token<'a>) -> Option<(Command<'a>, Token<'a>)> {
            ...
            .or_else(|| command_token.strip_prefix("\\"))?
        {
            ...
            "param" | "endcode" | "addtogroup" => Command::Skip,
            _ => return None,
        };
    ...
    fn inline_command<'a>(command_token: Token<'a>) -> Option<(Vec<&'a str>, Token<'a>)> {
        // println!("cargo:warning={:?}", command_token);
        ...
        // if *arg_token == " HealthServiceAccessibilityMaskAvailable" {
        //     panic!("aaa");
        // }
    ...
    impl<'a> Doc<'a> {
        ...
        fn body_token(&mut self, next_line: bool) -> Option<(Vec<&'a str>, bool)> {
            ...
            // println!("cargo:warning={:?}", next_token);
            match next_token
                .chars()
                .next()
    ...
    impl Doc<'_> {
        ...
        fn block(&mut self) -> bool {
            if let Some((command, end_token)) =
                self.tokens.peek().and_then(|token| block_type(token))
            {
                self.tokens.consume(end_token);
                match command {
